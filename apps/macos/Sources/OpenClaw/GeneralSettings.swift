...
struct GeneralSettings: View {
  ...
  private var isControlStatusDuplicate: Bool {
    guard case .failed(let message) = self.remoteStatus else { return false }
  ...
  private var healthCard: some View {
    ...
    return VStack(alignment: .leading, spacing: 6) {
      ...
      if let snap = snapshot {
        ...
        let linkLabel =
          linkId.flatMap { snap.channelLabels?[$0] } ?? linkId?.capitalized ?? "Link channel"
...
extension GeneralSettings {
  ...
  func testRemote() async {
    ...
    if self.state.remoteTransport == .direct {
      ...
      guard
        let sshCommand = Self.sshCheckCommand(
