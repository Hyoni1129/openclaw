...
extension CanvasWindowController {
  ...
  static func loadRestoredFrame(sessionKey: String) -> NSRect? {
    ...
    if rect.width < CanvasLayout.minPanelSize.width
      || rect.height < CanvasLayout.minPanelSize.height
    {
      return nil
    }
    return rect
  ...
  static func storeRestoredFrame(_ frame: NSRect, sessionKey: String) {
    ...
    UserDefaults.standard.set(
      [
        Double(frame.origin.x), Double(frame.origin.y), Double(frame.size.width),
