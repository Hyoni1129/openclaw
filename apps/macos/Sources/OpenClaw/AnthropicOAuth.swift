...
enum AnthropicAuthResolver {
  static func resolve(
    ...
    if let token = environment["ANTHROPIC_OAUTH_TOKEN"]?.trimmingCharacters(
...
enum AnthropicOAuth {
  ...
  static func exchangeCode(
    ...
    state: String,
    verifier: String
  ) async throws -> AnthropicOAuthCredentials {
    ...
    return AnthropicOAuthCredentials(
      type: "oauth", refresh: refresh, access: access, expires: expiresAtMs)
  }
...
enum OpenClawOAuthStore {
  ...
  static func legacyOAuthURLs() -> [URL] {
      ...
      !override.isEmpty
    {
      let expanded = NSString(string: override).expandingTildeInPath
      urls.append(
        URL(fileURLWithPath: expanded, isDirectory: true).appendingPathComponent(self.oauthFilename)
      )
    }
  ...
  static func loadAnthropicOAuthRefreshToken() -> String? {
    ...
    let refresh = self.firstString(
      in: rawEntry, keys: ["refresh", "refresh_token", "refreshToken"])
