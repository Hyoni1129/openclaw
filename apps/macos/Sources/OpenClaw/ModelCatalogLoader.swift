...
enum ModelCatalogLoader {
  ...
  private static var cachePath: URL {
    self.appSupportDir.appendingPathComponent(
      "model-catalog/models.generated.js", isDirectory: false)
  }
  ...
  static func load(from path: String) async throws -> [ModelChoice] {
    ...
    self.logger.debug(
      "model catalog load start file=\(URL(fileURLWithPath: resolved.path).lastPathComponent)")
    ...
    guard let rawModels = ctx?.objectForKeyedSubscript("MODELS")?.toDictionary() as? [String: Any]
    else {
    ...
    for (provider, value) in rawModels {
      ...
      for (id, payload) in models {
        ...
        choices.append(
          ModelChoice(id: id, name: name, provider: provider, contextWindow: ctxWindow))
      }
    }
