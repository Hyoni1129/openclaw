...
actor VoiceWakeRuntime {
  ...
  private func handleRecognition(_ update: RecognitionUpdate, config: RuntimeConfig) async {
    ...
    if let match {
      ...
      await self.beginCapture(
        command: match.command, triggerEndTime: match.triggerEndTime, config: config)
  ...
  private func isTriggerOnly(transcript: String, triggers: [String]) -> Bool {
    ...
    guard VoiceWakeTextUtils.startsWithTrigger(transcript: transcript, triggers: triggers) else {
      return false
    }
  ...
  private func preDetectSilenceCheck(
    ...
    guard self.lastTranscriptAt == lastSeenAt, self.lastTranscript == lastText else { return }
    guard
      let match = self.textOnlyFallbackMatch(
        transcript: lastText,
        triggers: triggers,
        config: gateConfig)
    else { return }
  ...
  private func beginCapture(command: String, triggerEndTime: TimeInterval?, config: RuntimeConfig)
    async
  {
  ...
  private static func makeAttributed(committed: String, volatile: String, isFinal: Bool)
    -> NSAttributedString
  {
