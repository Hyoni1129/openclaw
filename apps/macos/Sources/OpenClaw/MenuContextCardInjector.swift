...
final class MenuContextCardInjector: NSObject, NSMenuDelegate {
  ...
  private func refreshCache(force: Bool) async {
    if !force, let cacheUpdatedAt,
      Date().timeIntervalSince(cacheUpdatedAt) < self.refreshIntervalSeconds
    {
      return
    }
  ...
  private func cachedView() -> AnyView {
    ...
    return AnyView(
      ContextMenuCardView(rows: rows, statusText: self.cacheErrorText, isLoading: isLoading))
  }
