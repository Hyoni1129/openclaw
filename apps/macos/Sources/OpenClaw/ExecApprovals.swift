...
enum ExecApprovalsStore {
  ...
  static func addAllowlistEntry(agentId: String?, pattern: String) {
    ...
    self.updateFile { file in
      ...
      allowlist.append(
        ExecAllowlistEntry(pattern: trimmed, lastUsedAt: Date().timeIntervalSince1970 * 1000))
...
struct ExecCommandResolution: Sendable {
  ...
  static func resolve(command: [String], cwd: String?, env: [String: String]?)
    -> ExecCommandResolution?
  {
    guard let raw = command.first?.trimmingCharacters(in: .whitespacesAndNewlines), !raw.isEmpty
    else {
      return nil
    }
